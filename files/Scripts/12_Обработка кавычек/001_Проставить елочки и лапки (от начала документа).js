...
function Run() {
 ...
 var versionStr="Скрипт «Проставить елочки и лапки (от начала документа)» (версия 3.1)\nАвтор Sclex.\n\n";
 var otstupSverhu=60;
 var debug=false;
 ...
 function analyseSymbolOnTheLeft(ch) {
  ...
  else if (ch.search(re1)==0) {
   if (debug) alert("by symbol aleft - left quotes");
   return "left quotes";
  }
  else if (ch.search(re2)==0) {
   if (debug) alert("by symbol aleft - left quotes");
   return "right quotes";
  }
 }
 ...
 function analyseSymbolOnTheRight(ch) {
  if (ch==undefined) {
   if (debug) alert("by symbol aright - right quoutes");
   return "right quotes";
  }
  else if (ch.search(re3)==0) {
   if (debug) alert("by symbol aright - right quoutes");
   return "right quotes";
  }
  else if (ch.search(re4)==0) {
   if (debug) alert("by symbol aright - left quoutes");
   return "left quotes";
  }
 }
 ...
 window.external.BeginUndoUnit(document,"проставление елочек и лапок (от начала документа)");
