// Поиск некорректных маркеров концевых сносок v.1.4
...
if (result == null) {
   ...
   if (result !== null) {
   if (z == 0) {
     var textMsg = "Отсутствует самый первый в документе маркер текста сноски 1 и/или в документе нет ни одного маркера знака сноски." // #6";
      }
      else {
      var textMsg = "Отсутствует самый первый в документе маркер знака сноски 1. Ищите его выше выделенного места." // #6";
      }
      window.external.SetStatusBarText(textMsg); 
      result.index += result.length;
...
if (result == null) {
   ...
   if (result !== null) {
   if (t == 0) {
      var textMsg = "Маркер знака сноски без соответствующего ему маркера текста сноски. По-видимому у вас нет ни одного маркера текстов сносок." 
   }
   else {
...
if (result == null)  {
   ...
   for (var i = 0; i < page; i++) {
      ...
      if (zNaStr < tNaStr) {
          var countS = confirm("Количество  маркеров знаков и текстов сносок в документе не совпадают ("+zNaStr+" и "+tNaStr+" соответственно).\nПредлагаем перейти на: \n\n• Последний корректный МАРКЕР ЗНАКА сноски.\n(Нажмите кнопку OK).\n\n• Последний корректный МАРКЕР ТЕКСТА сноски.\n(Нажмите кнопку Отмена).\n\nВ большинстве случаев рекомендуется выбирать первый вариант (OK). Если вам не повезет, запустИте скрипт ещё раз и выберите второй вариант (Отмена).");
          if (countS == true) {
             count = mZ;
             markSnoski = "знака";
             };
 ...
 var numNote = ((mT-zNaStr-tNaStr)/2+tNaStr); // номер будущей сноски.
      ...
      var textMsg = "Количество маркеров знаков и текстов сносок в документе не совпадают. Вы перешли на последний выбранный вами корректный маркер " +markSnoski+ " БУДУЩЕЙ сноски № " +numNote+"." // #9";
      window.external.SetStatusBarText(textMsg); 
