...
//   * Чтобы скрипт смог распознать повторную обработку, в "истории изменений" должна остаться хотя бы одна запись сделанная этим скриптом (а именно — фрагмент текста "ГенУборка–" или "«Текст-корректор»").
 ...
 var SaveDef_on_off = 1;      // 0 ; 1 //      ("0" — отключить, "1" — включить)

// ---------------------------------------------------------------

         //  •  Выбор символа для апострофа  (№107)

 var Apostrof = 1;      // 0 ; 1 //      ("0" —  <'> <U+0027: Apostrophe>;  "1" —  <’> <U+2019: Ringht Single Quotation Mark> <Alt+0146>)
...
// -----------------------------
 ...
 if (SaveDef_on_off  == 1  &&  History != null)       //  Если включено сохранение дефиса...
         if (History.innerHTML.search(reSD)!=-1)       //  и в истории есть имя этого скрипта...
