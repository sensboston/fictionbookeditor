// Скрипт «Перейти на следующий абзац с искомым текстом» для редактора Fiction Book Editor (FBE).
// Версия 5.0
// Основано на скрипте "Поиск по набору регэкспов"
// Автор Sclex, сборка TaKir
// Скрипт производит переход на ближайший абзац с определенным текстом вниз по тексту.
...
function Run() {
 ...
 function init() {
  ...
  //  Тут впишите в скобках нужный вам текст или сочетание символов.

  addRegExp("^(.*?)(сказа)(.*?)$","i","Найдено: Слово или часть слова \\-сказа-\\.");

  //  Весь абзац с указанным текстом будет выделен и с ним можно выполнять необходимые действия с помощью горячих клавиш или скриптов.
 ...
 var errorList="";
 var checkTagStrRE=new RegExp("^(([-+](section|body|epigraph|annotation|cite|poem|stanza|title|subtitle|text-author))([ \t]+?[-+](section|body|epigraph|cite|poem|stanza|title|subtitle|text-author))*?|^$)$","i");
 var findTagRE=new RegExp("(^| )([-+])(section|body|epigraph|annotation|cite|poem|stanza|title|subtitle|text-author)(?= |$)","ig");
