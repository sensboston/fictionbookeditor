// Скрипт «Переход на следующий нумерованный или маркированный "список")» для редактора Fiction Book Editor (FBE).
// Версия 1.2
// Автор Sclex.
...
function Run() {
 ...
 function init() {
  ...
  addRegExp("(^[♠|♣|♦|♥|•|■|∙|▶|~\s](.*?)$)","","Найдено: Маркированный список - с графическими маркерами в начале строки");
 ...
 var errorList="";
 var checkTagStrRE=new RegExp("^(([-+](section|body|epigraph|cite|poem|stanza|title|subtitle|text-author|annotation))([ \t]+?[-+](section|body|epigraph|cite|poem|stanza|title|subtitle|text-author))*?|^$)$","i");
