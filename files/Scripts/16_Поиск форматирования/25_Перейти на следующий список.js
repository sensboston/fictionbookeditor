// Скрипт «Перейти на следующий нумерованный или маркированный "список"» для редактора Fiction Book Editor (FBE).
// Версия 5.1
// Основано на скрипте "Поиск по набору регэкспов"
// Автор Sclex, сборка TaKir
// Скрипт производит переход на ближайший "список" вниз по тексту.
...
function Run() {
 ...
 function init() {
  ...
  addRegExp("(^[0-9]{1,}[\)|\.](.*?)$)","","Найдено: Нумерованный список - с цифрами в начале строки");
  addRegExp("(^[a-zA-Zа-яА-Я][\)|\.](.*?)$)","","Найдено: Маркированный список - с буквами в начале строки");
  addRegExp("(^[♠♣♦♥•■∙▶~][\\x20\\xA0]{0,1}(.*?)$)","","Найдено: Маркированный список - с графическими маркерами в начале строки");
