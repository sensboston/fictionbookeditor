     ...
     for (i=1;i<=regExpCnt;i++) {
      ...
      if (checkAreWeInRightTags(i)) {
       ...
       while (rslt && flag1) {
        ...
        flag1=false;
        if (rslt) {
         newPos=s_html.substr(0,rslt.index).replace(imgTagRE,imgTagRE_).replace(removeTagsRE,removeTagsRE_).replace(shyRE,shyRE_).replace(ltRE,ltRE_).replace(gtRE,gtRE_).replace(nbspRE,nbspRE_).replace(ampRE,ampRE_).length;
         rslt_replaced=rslt[0].replace(imgTagRE,imgTagRE_).replace(removeTagsRE,removeTagsRE_).replace(shyRE,shyRE_).replace(ltRE,ltRE_).replace(gtRE,gtRE_).replace(nbspRE,nbspRE_).replace(ampRE,ampRE_);
