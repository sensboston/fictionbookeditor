// Скрипт «Поиск паразитной разрядки текста» для редактора Fiction Book Editor (FBE).
// Версия 5.1
// Автор Sclex, stokber
// Набор RegExp-ов - TaKir, 16.09.2024
// Основан на скрипте «Поиск по набору регэкспов (версия с исключениями)»
...
// -------------начало блока TaKir - регэкспы (TaKir, 16.09.2024)---------------


  //  _________________________________________________________________________
  //  Поиск "недопустимых" букв среди пробелов и знаков препинания
  //  (Могут быть ложные срабатывания на ФИО)
  //  _________________________________________________________________________

   addRegExp("( |□|▫|◦| )([гдеёзйлмнпртфхцчшщъыьэю])( |□|▫|◦| )","i","Найдено: недопустимая буква среди пробелов");
   addRegExp("^([гдеёзйлмнпртфхцчшщъыьэю]( |□|▫|◦| ))","i","Найдено: недопустимая буква в начале строки перед пробелом");
   addRegExp("( |□|▫|◦| )([гдеёзйлмнпртфхцчшщъыьэю])[\"-».,…:;!–-“”—«?]","i","Найдено: недопустимая буква между пробелом и знаком препинания");
   addRegExp("[\"»…“”«]([гдеёзйлмнпртфхцчшщъыьэю])( |□|▫|◦| )","i","Найдено: недопустимая буква между знаком препинания и пробелом");
   addRegExp("[-–-—]( |□|▫|◦| )([гдеёзйлмнпртфхцчшщъыьэю])( |□|▫|◦| )","i","Найдено: недопустимая буква между дефисами, тире и пробелом");



// -------------конец блока TaKir - регэкспы (TaKir, 16.09.2024)---------------
 ...
 function searchNext() {
   ...
   el=ptr;
   s=el.innerHTML.replace(removeTagsRE,removeTagsRE_).replace(imgTagRE,imgTagRE_).replace(ltRE,ltRE_).replace(gtRE,gtRE_).replace(nbspRE,nbspRE_).replace(ampRE,ampRE_);
   s_len=s.length;
     ...
     for (i=1;i<=regExpCnt;i++) {
      ...
      if (checkAreWeInRightTags(i)) {
       ...
       while (rslt && flag1) {
        ...
        flag1=false;
        if (rslt) {
         newPos=s_html.substr(0,rslt.index).replace(removeTagsRE,removeTagsRE_).replace(imgTagRE,imgTagRE_).replace(ltRE,ltRE_).replace(gtRE,gtRE_).replace(nbspRE,nbspRE_).replace(ampRE,ampRE_).length;
         rslt_replaced=rslt[0].replace(removeTagsRE,removeTagsRE_).replace(imgTagRE,imgTagRE_).replace(ltRE,ltRE_).replace(gtRE,gtRE_).replace(nbspRE,nbspRE_).replace(ampRE,ampRE_);
