// Скрипт «Перейти на следующую инлайн-иллюстрацию» v.1.0
// Является слегка изменённым скриптом «Следующая иллюстрация» за авторством Sсlex-а.
//Доведение до заявленного функционала — stokber (сентябрь 2025).

function Run() {
var name = "Перейти на следующую инлайн-иллюстрацию";
var vers ="1.0";
 
           /// НАСТРОЙКА ТИПА ВЫДЕЛЕНИЯ:
var sel = 0; // если 0, то курсор просто останавливается правее картинки ничего не выделяя, если любое другое число — будет выделен один символ для лучшей заметности. Картинку скрипт не выделяет!!!

// var rng=document.selection.createRange(); if(!rng) {alert("Установите курсор!");return false;}
...
// if(sel != 0) {shell.SendKeys("+{RIGHT "+sel+"}");}
