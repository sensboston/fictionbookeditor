// Скрипт «Расформатировать абзац(ы) от верхнего индекса»
// Автор Sclex
// Версия 1.5

function Run() {

 var undoMsg="расформатирование абзаца(-ев) от верхнего индекса";
 var statusBarMsg="Расформатируем абзац(ы) от верхнего индекса…";
 //имя тэга, который будет использован для маркеров начала и конца выделения
 var markerTagName="B";

 function processPs() {
  for (var i=0; i<ps.length; i++)
   ps[i].innerHTML=ps[i].innerHTML.replace(/<\/?(SUP)( [^>]*)?>/gi,"");
 }
