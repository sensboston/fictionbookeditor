// Скрипт «Сформатировать абзац(ы) верхним индексом»
// Автор Sclex
// Версия 1.5

function Run() {

 var undoMsg="форматирование абзаца(-ев) верхним индексом";
 var statusBarMsg="Форматируем абзац(ы) верхним индексом…";
 //имя тэга, который будет использован для маркеров начала и конца выделения
 var markerTagName="B";

 function processPs() {
  for (var i=0; i<ps.length; i++)
   ps[i].innerHTML="<SUP>"+ps[i].innerHTML.replace(/<\/?(SUP)( [^>]*)?>/gi,"")+"</SUP>";
 }
