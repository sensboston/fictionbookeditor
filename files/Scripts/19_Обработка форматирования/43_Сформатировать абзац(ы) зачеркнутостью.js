// Скрипт «Сформатировать абзац(ы) зачеркнутостью»
// Автор Sclex
// Версия 1.5

function Run() {

 var undoMsg="форматирование абзаца(-ев) зачеркнутостью";
 var statusBarMsg="Форматируем абзац(ы) зачеркнутостью…";
 //имя тэга, который будет использован для маркеров начала и конца выделения
 var markerTagName="B";

 function processPs() {
  for (var i=0; i<ps.length; i++)
   ps[i].innerHTML="<STRIKE>"+ps[i].innerHTML.replace(/<\/?(STRIKE)( [^>]*)?>/gi,"")+"</STRIKE>";
 }
