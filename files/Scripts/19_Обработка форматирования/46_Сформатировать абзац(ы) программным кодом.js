// Скрипт «Сформатировать абзац(ы) программным кодом»
// Автор Sclex
// Версия 1.5

function Run() {

 var undoMsg="форматирование абзаца(-ев) программным кодом";
 var statusBarMsg="Форматируем абзац(ы) программным кодом…";
 ...
 function processPs() {
  ...
  for (var i=0; i<ps.length; i++) {
   spans=ps[i].getElementsByTagName("SPAN");
   for (j=spans.length-1; j>=0; j--)
    if (spans[j].className=="code") spans[j].removeNode(false);
   ps[i].innerHTML="<SPAN class=code>"+ps[i].innerHTML.replace(/<\/?SUB( [^>]*)?>/gi,"")+"</SPAN>";
  }
 }
