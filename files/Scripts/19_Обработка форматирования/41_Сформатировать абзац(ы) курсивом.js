// Скрипт «Сформатировать абзац(ы) курсивом»
// Автор Sclex
// Версия 1.5

function Run() {

 var undoMsg="форматирование абзаца(-ев) курсивом";
 var statusBarMsg="Форматируем абзац(ы) курсивом…";
 //имя тэга, который будет использован для маркеров начала и конца выделения
 var markerTagName="B";

 function processPs() {
  for (var i=0; i<ps.length; i++)
   ps[i].innerHTML="<EM>"+ps[i].innerHTML.replace(/<\/?(EM|I)( [^>]*)?>/gi,"")+"</EM>";
 }
