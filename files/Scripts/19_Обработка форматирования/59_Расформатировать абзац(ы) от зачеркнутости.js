// Скрипт «Расформатировать абзац(ы) от зачеркнутости»
// Автор Sclex
// Версия 1.5

function Run() {

 var undoMsg="расформатирование абзаца(-ев) от зачеркнутости";
 var statusBarMsg="Расформатируем абзац(ы) от зачеркнутости…";
 //имя тэга, который будет использован для маркеров начала и конца выделения
 var markerTagName="B";

 function processPs() {
  for (var i=0; i<ps.length; i++)
   ps[i].innerHTML=ps[i].innerHTML.replace(/<\/?(STRIKE)( [^>]*)?>/gi,"");
 }
