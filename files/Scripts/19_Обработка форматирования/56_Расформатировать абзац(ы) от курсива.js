// Скрипт «Расформатировать абзац(ы) от курсива»
// Автор Sclex
// Версия 1.5

function Run() {

 var undoMsg="Расформатирование абзаца(-ев) от курсива";
 var statusBarMsg="Расформатируем абзац(ы) от курсива…";
 //имя тэга, который будет использован для маркеров начала и конца выделения
 var markerTagName="B";

 function processPs() {
  for (var i=0; i<ps.length; i++)
   ps[i].innerHTML=ps[i].innerHTML.replace(/<\/?(EM|I)( [^>]*)?>/gi,"");
 }
