// Скрипт «Сформатировать абзац(ы) курсивом и жирностью»
// Автор Sclex
// Версия 1.5

function Run() {

 var undoMsg="форматирование абзаца(-ев) курсивом и жирностью";
 var statusBarMsg="Форматируем абзац(ы) курсивом и жирностью…";
 //имя тэга, который будет использован для маркеров начала и конца выделения
 var markerTagName="STRIKE";

 function processPs() {
  for (var i=0; i<ps.length; i++)
   ps[i].innerHTML="<STRONG><EM>"+ps[i].innerHTML.replace(/<\/?(EM|I|STRONG|B)( [^>]*)?>/gi,"")+"</EM></STRONG>";
 }
