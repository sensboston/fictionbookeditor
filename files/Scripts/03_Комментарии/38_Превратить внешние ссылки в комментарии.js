//======================================
//            «Превратить внешние ссылки в комментарии» 1.3 (декабрь, 2024)
//
// Сценарий представляет собой компиляцию из незначительно 
// изменённых двух скриптов за авторством Sclex-а: 
// "Генеральная уборка" и "Комментарии из скобок".
// Автор сборки — stokber
//~~~~~~~~~~~~~~~~~~
...
// в  переменной fromText: 
var Metk = (fromText.match(/【♣★|★♣】/g) || []).length;
if (Metk > 0) {
   window.clipboardData.setData("text","【♣★|★♣】");
   alert("Это невероятно, но в вашем документе обнаружились, невесть как туда попавшие, редкие сочетания трех редких символов '【♣★' и(ли) '★♣】', которые используются скриптом как временные метки будущих сносок. Найти в документе их можно вставив регексп из буфера обмена в строку 'Что искать:' окна 'Поиск' (Ctrl+F). Не забудьте включить проверку регулярных выражений.\nДля корректной работы сценария рекомендуем на время работы скрипта заменить эти случайные символы какими-нибудь другими или вставьте между этих трёх символов какие-либо другие символы, например так: '【+♣+★'.")
   return true;
} 
...
/* var openBracket1 = "[~"; // обычным текстом
var closBracket1 = "~]";
var openBracket2 = "\\[~"; // регекспом
var closBracket2 = "~\\]"; */

var openBracket1 = "【♣★"; // обычным текстом "【♣★"
var closBracket1 = "★♣】"; // обычным текстом "★♣】"
var openBracket2 = "【♣★"; // регекспом "【♣★"
var closBracket2 = "★♣】"; // регекспом "★♣】"

function Run1() {  // перенос адреса ссылки в скобки.
...
// var re100 = new RegExp("<A href=\\\"([^>]+)\\\">(.+?)</a>","gi");
// var re100 = new RegExp("<A href=\\\"((?:(?:https?|ftp|file)://|www\\.|ftp\\.).+)\\\">(.+?)</a>","gi");
var re100 = new RegExp("<A (?:class=\\\"\\\" )?href=\\\"((?:https?:|ftp:|file:|mailto:).+?)\\\">(.+?)</A>","gi");
// (www\\\.|mailto:|mail.*|.*@.*|file.*|www.*)
// var re100_ = "$2[~$1~]";
var re100_ = "$2"+openBracket1+"$1"+closBracket1;
var count_100 = 0;
 ...
 function HandleP(ptr) {
  ...
  s=ptr.innerHTML;
   ...
   ptr.innerHTML=s;      
    ...
    window.external.BeginUndoUnit(document,"«адрес ссылки в скобки»");                               // ОТКАТ (UNDO) начало
...
//без тире в диалогах 08 и дублей пробелов 80
 ...
 // if (count_100!=0)   {st2+='\n• адресов в скобки:                             	'+count_100;}
 

 // if (st2!="") st2="\n"+st2;

 /* MsgBox ('                  –= Jurgen Script =– \n'+
        ' «Адрес сноски в скобки» v.'+VersionNumber+' stokber Edition       \n\n'+
...
function Run2() {  // Комментарии из скобок.
...
var AddSnoska=true;
//показывать ли форму для ввода текста комментария
//true - показывать. false - не показывать
var inputSnoskaText=false;
//перемещать ли фокус видимости на раздел свежесозданного комментария
var moveFocusToNote=false;
var strconst1="c_%N"; //название раздела комментария
var snoskaHrefRegExp=new RegExp("^c_","");
var strconst2="<P>%N</P>"; // заголовок раздела комментария
var strconst3="&nbsp;"; //содержание раздела комментария
...
var otstupSverhu=40;
//задает name того боди, которое скрипт будет считать за боди комментарий
var bodyNotesName="comments";
...
Masks["0_1"]=new RegExp(openBracket2,""); //открывающая скобка
//регулярное выражения для поиска закрывающей скобки
Masks["0_2"]=new RegExp(closBracket2,""); //закрывающая скобка
...
function getVersionStr() {
 return("'Превратить внешние ссылки в комментарии' v"+versionNumber+".\n\n");
}
...
function getExitStr(withoutMinusOne) {
 ...
 if (withoutMinusOne!=undefined)
  return "Добавлено комментариев: "+NewNotesCnt+".\nЗатрачено времени: "+TimeStr;
 else
  return "Добавлено комментариев: "+(NewNotesCnt-1)+".\nЗатрачено времени: "+TimeStr;
}
...
// в цикле переберем все DIV-ы и найдем body notes
 ...
 if (NashliBodyNotes) {
   ...
   if (SecondLevelSection) {
     MsgBox("В body комментариев есть разделы второго уровня вложения. Такие файлы не обрабатываются данным скриптом. Работа скрипта завершена.");
     return;
   }
...
function dobavlenie_snoski(NoteNum) {
  ...
  //прочитаем в массив SectID ID-ы разделов комментариев
  ...
  if (j5<document.links.length) {
   ...
   while (while_flag) {
    if (j6<document.links.length)
     if (getLocalHref(document.links[j6].getAttribute("href"))==null) j6++;
     else if (getLocalHref(document.links[j6].getAttribute("href")).search(snoskaHrefRegExp)<0) 
      j6++;
   ...
   if (j6<document.links.length) {
    ...
    if (!BodyNotes.contains(utb) || utb.className!="section") {
     ...
     document.links[j5].scrollIntoView(true);
     finalization();
     MsgBox(getVersionStr()+"Ссылка сноски ссылается не внутрь раздела в теле комментариев.\nПозиция обзора установлена на неисправную ссылку."+"\n\n"+getExitStr());
     errorStatus=1;
     window.external.EndUndoUnit(document);
     return;
    }
...
function finalization() {
 ...
 // поменяем заголовки разделов комментариев
 var sectCnt=0;
 ...
 //    if (document.links[j2].className=="note") {
  ...
  if (href!=null) {
   if (sectNumById[href]) {
    ...
    // меняем текст ссылки
    if (getLocalHref(document.links[j2].getAttribute("href")).search(snoskaHrefRegExp)>=0)
   ...
   while (iii1<MasksN) {
    ...
    //сюда попадаем, если нашли открывающую скобку
      ...
      if ((srch3==-1 && srch5!=-1)||(srch1!=-1 && srch5!=-1 && srch5<srch3)) {
       ...
       MsgBox(getVersionStr()+"После открывающей скобки встречена другая открывающая, видимо пропущена закрывающая. Курсор установлен перед комментарием с ошибкой."+"\n\n"+getExitStr());
       window.external.EndUndoUnit(document);
       return;
      }
      if (srch3!=-1) {
      ...
      //т.е. это внутренний цикл прохода по дереву
       if (ptr && ptr.firstChild!=null) {
        ...
        while (ptr && ptr.nextSibling==null) {
         ...
         if (level==-2 || (level==-1 && ptr.nodeName!="P" && ptr.nodeName!="DIV")) {
          ...
          MsgBox(getVersionStr()+"Комментарий кончается не внутри того тэга, в котором начинается. Курсор установлен перед комментарием с ошибкой.\n\n"+getExitStr());
          window.external.EndUndoUnit(document);
          return;
         }
       ...
       // GoTo(CurrentP);
       // MsgBox(getVersionStr()+"Ошибка. Недопустимое форматирование внутри скобок комментария. Курсор установлен на абзац, где начинается комментарий с ошибкой.\n\n"+getExitStr());
       // window.external.EndUndoUnit(document);
       // return;
       //}
      //ELEMENT_NODE
       ...
       if (ptr.nodeType==3) {
        ...
        if (srch2!=-1) {
         if (level>0) {
          finalization();
          setCursorIntoTextNode(beginNode,srch1);
          MsgBox(getVersionStr()+"Ошибка. Не все тэги, открытые внутри скобок, в них закрываются. Курсор установлен перед комментарием с ошибкой.\n\n"+getExitStr());
          window.external.EndUndoUnit(document);
          return;
         }
...
function getRandomNum(n) {
 ...
 return s;
    ...
    window.external.BeginUndoUnit(document,"создание комментариев из внешних ссылок");                               // ОТКАТ (UNDO) начало
Run1();
Run2();
    window.external.EndUndoUnit(document);                                             // undo конец
