...
function Run() {
 ...
 while (ptr) {
  if (ptr.nodeName=="DIV" && ptr.className=="section") {
   ...
   if (id!="") {
    ...
    MsgBox("Раздел номер "+SectCnt+" в body комментариев не имеет id.\n"+
           "Перемещаемся на нее...");
    GoTo(ptr);
    return;
   }
 ...
 if (ErrTxt!="") {
  MsgBox("Отсутствуют ссылки на некоторые разделы комментариев.\n"+
 ...
 MsgBox("Проверка корректности оформления комментариев завершена. Ошибок не обнаружено.\n"+
