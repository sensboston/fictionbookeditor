// Скрипт «Поиск возможных заголовков по ключевым словам» для редактора Fiction Book Editor (FBE).
// Версия 5.0
// Основано на скрипте «Поиск по набору регэкспов» для редактора Fiction Book Editor (FBE).
// Автор исходного скрипта Sclex, набор RegExp-ов - TaKir, Sclex, апрель, 2025
// С комментариями и исправлениями от Александра Ка, апрель, 2025
...
// -------------начало блока TaKir - регэкспы (03.04.2025)---------------
  ...
  //  Ищем потенциальные заголовки по ключевым словам

  //  Вводим три переменные (с учетом рода) для числительных прописью

 var pervaja = "первая|вторая|третья|четв[её]ртая|пятая|шестая|седьмая|восьмая|девятая|десятая|одиннадцатая|двенадцатая|тринадцатая|четырнадцатая|пятнадцатая|шестнадцатая|семнадцатая|восемнадцатая|девятнадцатая|двадцатая|двадцать|тридцатая|тридцать|сороковая|сорок|пятидесятая|пятьдесят|шестидесятая|шестьдесят|семидесятая|семьдесят|восьмидесятая|восемьдесят|девяностая|девяносто|сотая|сто|двухсотая|двести|тр[её]хсотая|триста|четыр[её]хсотая|четыреста|пятисотая|пятьсот|шестисотая|шестьсот|семисотая|семьсот|восьмисотая|восемьсот|девятисотая|девятьсот|тысячная|тысяча";

 var pervyj = "первый|второй|третий|четв[её]ртый|пятый|шестой|седьмой|восьмой|девятый|десятый|одиннадцатый|двенадцатый|тринадцатый|четырнадцатый|пятнадцатый|шестнадцатый|семнадцатый|восемнадцатый|девятнадцатый|двадцатый|двадцать|тридцатый|тридцать|сороковой|сорок|пятидесятый|пятьдесят|шестидесятый|шестьдесят|семидесятый|семьдесят|восьмидесятый|восемьдесят|девяностый|девяносто|сотый|сто|двухсотый|двести|тр[её]хсотый|триста|четыр[её]хсотый|четыреста|пятисотый|пятьсот|шестисотый|шестьсот|семисотый|семьсот|восьмисотый|восемьсот|девятисотый|девятьсот|тысячный|тысяча";

 var pervoe = "первое|второе|третье|четв[её]ртое|пятое|шестое|седьмое|восьмое|девятое|десятое|одиннадцатое|двенадцатое|тринадцатое|четырнадцатое|пятнадцатое|шестнадцатое|семнадцатое|восемнадцатое|девятнадцатое|двадцатое|двадцать|тридцатое|тридцать|сороковое|сорок|пятидесятое|пятьдесят|шестидесятое|шестьдесят|семидесятое|семьдесят|восьмидесятое|восемьдесят|девяностое|девяносто|сотое|сто|двухсотое|двести|тр[её]хсотое|триста|четыр[её]хсотое|четыреста|пятисотое|пятьсот|шестисотое|шестьсот|семисотое|семьсот|восьмисотое|восемьсот|девятисотое|девятьсот|тысячное|тысяча";   //  (для "Первое действие")
 

  //  1. Ищем заголовки типа "Первая глава", "Первая часть", "Первая книга",
  //  "Первая сцена", "Первая таблица"

   addRegExp("^[ \xA0]*(("+pervaja+")[ \xA0]+)+глава([ \xA0\\.,]|$)", "i", "Найдено: Возможные главы", "-title -subtitle");
   addRegExp("^[ \xA0]*(("+pervaja+")[ \xA0]+)+часть([ \xA0\\.,]|$)", "i", "Найдено: Возможные части", "-title -subtitle");
   addRegExp("^[ \xA0]*(("+pervaja+")[ \xA0]+)+книга([ \xA0\\.,]|$)", "i", "Найдено: Возможные книги", "-title -subtitle");
   addRegExp("^[ \xA0]*(("+pervaja+")[ \xA0]+)+сцена([ \xA0\\.,]|$)", "i", "Найдено: Возможные сцены", "-title -subtitle");
   addRegExp("^[ \xA0]*(("+pervaja+")[ \xA0]+)+таблица([ \xA0\\.,]|$)", "i", "Найдено: Возможные таблицы", "-title -subtitle");


  //  2. Ищем заголовки типа "Первый том", "Первый раздел", "Первый эпизод",
  //  "Первый урок", "Первый параграф",
 
   addRegExp("^[ \xA0]*(("+pervyj+")[ \xA0]+)+том([ \xA0\\.,]|$)", "i", "Найдено: Возможные тома", "-title -subtitle");
   addRegExp("^[ \xA0]*(("+pervyj+")[ \xA0]+)+раздел([ \xA0\\.,]|$)", "i", "Найдено: Возможные разделы", "-title -subtitle");
   addRegExp("^[ \xA0]*(("+pervyj+")[ \xA0]+)+эпизод([ \xA0\\.,]|$)", "i", "Найдено: Возможные эпизоды", "-title -subtitle");
   addRegExp("^[ \xA0]*(("+pervyj+")[ \xA0]+)+урок([ \xA0\\.,]|$)", "i", "Найдено: Возможные уроки", "-title -subtitle");
   addRegExp("^[ \xA0]*(("+pervyj+")[ \xA0]+)+параграф([ \xA0\\.,]|$)", "i", "Найдено: Возможные параграфы", "-title -subtitle");


  //  3. Ищем заголовки типа "Первое действие", "Первое задание", "Первое приложение",

   addRegExp("^[ \xA0]*(("+pervoe+")[ \xA0]+)+действие([ \xA0\\.,]|$)", "i", "Найдено: Возможные действия", "-title -subtitle");
   addRegExp("^[ \xA0]*(("+pervoe+")[ \xA0]+)+задание([ \xA0\\.,]|$)", "i", "Найдено: Возможные задания", "-title -subtitle");
   addRegExp("^[ \xA0]*(("+pervoe+")[ \xA0]+)+приложение([ \xA0\\.,]|$)", "i", "Найдено: Возможные приложения", "-title -subtitle");


  //  4. Ищем заголовки только по ключевым словам - из одного слова


addRegExp("^(Глава|ГЛАВА|Часть|ЧАСТЬ|Раздел|РАЗДЕЛ|Том|ТОМ|Пролог|ПРОЛОГ|Эпилог|ЭПИЛОГ|Предисловие|ПРЕДИСЛОВИЕ|Послесловие|ПОСЛЕСЛОВИЕ|Введение|ВВЕДЕНИЕ|Вступление|ВСТУПЛЕНИЕ|Заключение|ЗАКЛЮЧЕНИЕ|Приложения|ПРИЛОЖЕНИЯ|Приложение|ПРИЛОЖЕНИЕ|Интерлюдия|ИНТЕРЛЮДИЯ|Благодарности|БЛАГОДАРНОСТИ|Словарь|СЛОВАРЬ|Список|СПИСОК|Перечень|ПЕРЕЧЕНЬ|Индекс|ИНДЕКС|Литература|ЛИТЕРАТУРА|Библиография|БИБЛИОГРАФИЯ|Акт|АКТ|Сцена|СЦЕНА|Таблица|ТАБЛИЦА|Параграф|ПАРАГРАФ|Выводы|ВЫВОДЫ|Эпизод|ЭПИЗОД|Действие|ДЕЙСТВИЕ|Иллюстрации|ИЛЛЮСТРАЦИИ|Примечания|ПРИМЕЧАНИЯ|Указатель|УКАЗАТЕЛЬ|Глоссарий|ГЛОССАРИЙ|Термины|ТЕРМИНЫ|Посвящение|ПОСВЯЩЕНИЕ|Интермедия|ИНТЕРМЕДИЯ|Персонажи|ПЕРСОНАЖИ|Оглавление|ОГЛАВЛЕНИЕ|Урок|УРОК|Уроки|УРОКИ|Задание|ЗАДАНИЕ|Задания|ЗАДАНИЯ|Задача|ЗАДАЧА|Задачи|ЗАДАЧИ|Ответ|ОТВЕТ|Ответы|ОТВЕТЫ|Part|PART|Chapter|CHAPTER|Volume|VOLUME|Book|BOOK|Stave|STAVE|Prolog|PROLOG|Prologue|PROLOGUE|Epilog|EPILOG|Epilogue|EPILOGUE|Foreword|FOREWORD|Afterword|AFTERWORD|Preface|PREFACE|Pov|POV|Introduction|INTRODUCTION|Dedication|DEDICATION|Guides|GUIDES|Bibliography|BIBLIOGRAPHY|Biography|BIOGRAPHY|Summary|SUMMARY|Playlist|PLAYLIST|Lesson|LESSON|Lessons|LESSONS|Unit|UNIT|Appendix|APPENDIX|Appendices|APPENDICES|Vocabulary|VOCABULARY|Questions|QUESTIONS|Test|TEST|Figure|FIGURE|Module|MODULE|Modules|MODULES|Contents|CONTENTS|Act|ACT|Scene|SCENE)([ \xA0\\.,]|$)","","Найдено: Возможные заголовки","-title -subtitle");


  //  5. Ищем заголовки только по ключевым словам - из двух и более слов

addRegExp("^(Об авторе|ОБ АВТОРЕ|От автора|ОТ АВТОРА|От редакции|ОТ РЕДАКЦИИ|Рекомендуемая литература|РЕКОМЕНДУЕМАЯ ЛИТЕРАТУРА|Используемая литература|ИСПОЛЬЗУЕМАЯ ЛИТЕРАТУРА|Список литературы|СПИСОК ЛИТЕРАТУРЫ|Список используемой литературы|СПИСОК ИСПОЛЬЗУЕМОЙ ЛИТЕРАТУРЫ|Список использованной литературы|СПИСОК ИСПОЛЬЗОВАННОЙ ЛИТЕРАТУРЫ|Действующие лица|ДЕЙСТВУЮЩИЕ ЛИЦА|От составителей|ОТ СОСТАВИТЕЛЕЙ|От издательства|ОТ ИЗДАТЕЛЬСТВА|Список сокращений|СПИСОК СОКРАЩЕНИЙ|Список аббревиатур|СПИСОК АББРЕВИАТУР|Указатель имен|УКАЗАТЕЛЬ ИМЕН|Список иллюстраций|СПИСОК ИЛЛЮСТРАЦИЙ|Библиографический указатель|БИБЛИОГРАФИЧЕСКИЙ УКАЗАТЕЛЬ|Алфавитный указатель|АЛФАВИТНЫЙ УКАЗАТЕЛЬ|Список ошибок и опечаток|СПИСОК ОШИБОК И ОПЕЧАТОК|Предисловие издателя|ПРЕДИСЛОВИЕ ИЗДАТЕЛЯ|Предисловие переводчика|ПРЕДИСЛОВИЕ ПЕРЕВОДЧИКА|Supplementary Material|SUPPLEMENTARY MATERIAL)","","Найдено: Возможные заголовки - из нескольких слов","-title -subtitle");


// -------------конец блока TaKir - регэкспы (03.04.2025):---------------
