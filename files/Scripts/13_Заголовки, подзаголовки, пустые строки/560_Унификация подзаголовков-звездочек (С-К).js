//======================================
//             «Скрипт-конструктор» версия «Унификация подзаголовков-звёздочек» (TaKir, декабрь 2024)
...
var NazvanieSkripta = "Унификация подзаголовков-звёздочек";      // текст //
...
// ---------------------------------------------------------------
         ...
         //     Выбор направленности скрипта:   Поиск и подсчёт   или   Исправления
//  (выбор заголовка для раздела с результатами обработки)

var Poisk_ili_Ispravlenija = 1;      // 0 ; 1 //      ("0" — Поиск и подсчет, "1" — Исправления)
...
var Poslovitsa_on_off = 0;      // 0 ; 1 //      ("0" — НЕ добавлять, "1" — добавлять)
...
//  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ///  Сюда можно добавлять новые операции по поиску/замене.
//  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

 n = 1;   //  унификация более 3 звёздочек с разными пробелами или без (TaKir)
   if (nachalo) {
             re[n] = new RegExp("^(?:"+sIB+")?(?:(?:\\\s|"+nbspEntity+"){0,}[*]{1,}(?:\\\s|"+nbspEntity+"){0,}){3,}(?:"+fIB+")?$","g");
             re_[n] = "* * *";
           stroka_v_statistike[n] = "<Унификация более 3 звёздочек (TaKir)>   .  .  .  .  .  .  .  .  .	";
           }
   if (subtitle)      //  annotation, history, title, subtitle, poem, stanza, cite, epigraph, table, text_author, text
   if (s.search(re[n]) != -1) {
...
//  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  /* /// начало большого комментария
                 ...
                 ///  ФОРМА ЗАПИСИ ДЛЯ ОПЕРАЦИИ ПОИСКА/ЗАМЕНЫ (шаблон)

   n = 1;   //  унификация более 3 звездочек в подзаголовках (TaKir)
   if (nachalo) {
           re[n] = new RegExp("([\\*]){4,}","gi");
           re_[n] = "* * *";
...
Кроме того в регулярные выражения можно добавлять переменные, которые могут значительно расширить возможности поиска и замены.
...
Поэтому в регулярное выражение для поиска   вместо символа неразрывного пробела следует вставлять переменную ("+nbspEntity+"), а в регулярное выражение для замены   вместо символа неразрывного пробела следует вставлять переменную ("+nbspChar+").

•  re_[n] = "Регулярное выражение для замены";
В этой строке записывается регулярное выражение для замены.
Здесь удвоение символа «\»  не нужно.
Можно добавлять переменные.
Если в операции будет производиться только поиск, то эта строка удаляется.

Пример строк поиска и замены:
(Буква или кавычки + тире + обычный или н/р пробел   ›››   Буква или кавычки + короткое тире + пробел)
re[n] = new RegExp("([А-яA-Za-zЁё“»”\\\"])("+fIB+"){0,1}—("+fIB+"){0,1}(\\\s|"+nbspEntity+")","g");
re_[n] = "$1$2–$3 ";
...
Внутрь скобок оператора условия «if» добавляется переменная, имя которой совпадает с названием раздела, в котором может быть замена (или поиск).
...
Если область поиска/замены не должна включать в себя некий формат, то название предваряется оператором отрицания «!»
например:  if (! annotation)
Если область поиска/замены включает в себя несколько форматов, то названия следует записывать с использованием оператора логического сложения «||».
например:  if (annotation  ||  epigraph)
Если область поиска/замены НЕ включает в себя несколько форматов, то названия следует записывать с использованием оператора логического умножения «&&».
например:  if (! annotation  &&  ! epigraph)
Если область поиска/замены ничем не ограничена, то эта строка просто удаляется.
...
Здесь ничего изменять не требуется.
Но если в операции производится только поиск, то эта строка удаляется.
...
//   Неразрывные пробелы:  ("+nbspEntity+") - для формулы поиска  ;  ("+nbspChar+") - для формулы замены  ;
 try { var nbspChar=window.external.GetNBSP(); var nbspEntity; if (nbspChar.charCodeAt(0)==160) nbspEntity="&nbsp;"; else nbspEntity=nbspChar }
 catch(e) { var nbspChar=String.fromCharCode(160); var nbspEntity="&nbsp;" }

//   Теги курсива и жирности
//   ("+sIB+")  — начальный тег курсива, жирности или жирного курсива.
//   ("+fIB+")  — конечный тег курсива, жирности или жирного курсива.
//   ("+aIB+")  — или начальный, или конечный тег курсива, жирности, жирного курсива.
...
// определение форматирования строки
   ...
   if (ptr.innerHTML != s) {
           count_GU++;          // Счетчик изменений строк
           ptr.innerHTML=s }
...
// -----------------------------
 ...
 window.external.BeginUndoUnit(document,"«"+NazvanieSkripta+"» v."+NumerusVersion);  // Начало записи в систему отмен FBE
...
//  * При добавлении в функцию (HandleP) операции удаления или добавление полностью всего узла <P>, необходимо в этой операции корректировать длину массива (mPLength) и номер элемента (jj).
...
// -----------------------------
 ...
 var cTaT1=ind-1;  //  число строк в первом разделе
                                                             ...
                                                             mSt[ind]="					";  ind++;
 if (Poisk_ili_Ispravlenija==0)  mSt[ind]="• ПОИСК И ПОДСЧЁТ:			";
     else                                                mSt[ind]="• ИСПРАВЛЕНИЯ:				";
                                                             ind++;
