...
// Версия 1.2
// Автор Sclex, набор RegExp-ов - TaKir, Sclex, 22.11.2024

// Редакция на 22.11.2024.
...
// -------------начало блока TaKir - регэкспы (TaKir, 22.11.2024)---------------

    
  addRegExp("^[\]]","","Найдено: скобки в начале строки");
  addRegExp("^[\*]","","Найдено: звездочки в начале строки", "-subtitle");

// addRegExp("([A-Za-zА-яЁё])([\\x20\\xA0])?[\]\*]","","Найдено: звездочки или скобки после букв и знаков препинания");
// addRegExp("[\]\*]([\\x20\\xA0])?[A-Za-zА-яЁё]","","Найдено: звездочки или скобки перед буквами");


  addRegExp("([A-Za-zА-яЁё…,;.»\"!?:%“™”©\\)])([\\x20\\xA0])?[\*]","","Найдено: звездочки после букв и знаков препинания");
  addRegExp("[\*]([\\x20\\xA0])?[A-Za-zА-яЁё]","","Найдено: звездочки или скобки перед буквами");


  addRegExp("([A-Za-zА-яЁё…,;.»\"!?:%“™”©\\)])([0-9])([0-9])([0-9])([0-9])","","Найдено: 4 цифры, прилипшие к словам и знакам препинания");
  addRegExp("([A-Za-zА-яЁё…,;.»\"!?:%“™”©\\)])([0-9])([0-9])([0-9])","","Найдено: 3 цифры, прилипшие к словам и знакам препинания");
  addRegExp("([A-Za-zА-яЁё…,;.»\"!?:%“™”©\\)])([0-9])([0-9])","","Найдено: 2 цифры, прилипшие к словам и знакам препинания");
  addRegExp("([A-Za-zА-яЁё…,;.»\"!?:%“™”©\\)])([0-9])","","Найдено: 1 цифра, прилипшая к словам и знакам препинания");





// -------------конец блока TaKir - регэкспы (TaKir, 22.11.2024)---------------
    ...
    if (el.nodeName=="P" && (s1_len<s_len || s_len==0)) {
     ...
     while (currFound<foundsCnt) {
       ...
       if (!(ignoreNullPosition && founds[currFound].pos==s1_len)) {
		...
		/* //  позиция и длина совпадения:
		    alert("Позиция: "+foundPos+"\nДлина: "+foundLen+"\nНовая позиция: "+s1_len);
			alert(s_html);
			alert(s); */
