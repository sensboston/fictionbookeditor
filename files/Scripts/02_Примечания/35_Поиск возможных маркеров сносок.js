// Скрипт «Поиск возможных маркеров сносок» для редактора Fiction Book Editor (FBE).
// Версия 1.0 (20.02.2022)
// Автор исходного скрипта "Поиск по набору регэкспов" - Sclex.
...
// -------------начало блока регэкспы (20.02.2022)---------------

tagRegExp("(?<=[\\x20\\xA0\\t\\n\\r\\f])<sup>\\d+?</sup>","","Найдено: Пробел перед маркером сноски, возможно приклеившийся абзац текста сноски");
tagRegExp("^<sup>.+?</sup>","","Найдено: Верхний индекс в начале абзаца");
tagRegExp("(?<!^)<sup>.+?</sup>","","Найдено: Верхний индекс в середине абзаца");


// Это все, что будет с тегами. Остальное — addRegExp-ы. Это, по сути, те же регулярки, только косую черту надо будет удвоить.


addRegExp("(?<=[A-Za-zА-яЁё])\\d+","i","Найдено: Прилипшие к тексту цифры");
addRegExp("(?<=[.,:;\\)»”“?!…\"])\\d+","","Найдено: Прилипшие к знакам препинания цифры");
addRegExp("(?<=[A-Za-zА-яЁё])[’＇'`‘՚′ʹꞌꞋ᾿°]+","i","Найдено: Прилипшие к тексту апострофы");
addRegExp("(?<=[0-9])[’＇'`‘՚′ʹꞌꞋ᾿°]+","i","Найдено: Прилипшие к цифрам апострофы");
addRegExp("(?<=[A-Za-zА-яЁё])[\\x20\\xA0\\t\\n\\r\\f][’＇'`‘՚′ʹꞌꞋ᾿°]+","i","Найдено: Текст с пробелом и апострофом");
addRegExp("(?<=[0-9])[\\x20\\xA0\\t\\n\\r\\f][’＇'`‘՚′ʹꞌꞋ᾿°]+","i","Найдено: Цифры с пробелом и апострофом");
addRegExp("(?<=[A-Za-zА-яЁё])[*]+","i","Найдено: Прилипшие к тексту звездочки");
addRegExp("(?<=[A-Za-zА-яЁё])[»….,:;!\\?-—–][*]+","i","Найдено: Звездочки после знаков препинания");
addRegExp("(?<=[A-Za-zА-яЁё])[\\x20\\xA0\\t\\n\\r\\f][\\*]+","i","Найдено: Текст с пробелом и звездочками");
addRegExp("^[*]","","Найдено: звездочки в начале строки, но не сабтайтлы","-subtitle");




// -------------конец блока регэкспы (20.02.2022)---------------
