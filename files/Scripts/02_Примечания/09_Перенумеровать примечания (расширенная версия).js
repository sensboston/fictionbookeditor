function Run() {
  ...
  // Унификация сносок (объединенная)
  ...
  */
  ...
  // снятие курсива со [сноски]
   var re01 = new RegExp("(<A class=note [^<]+?</A>)("+fIB+")","gi");
   var re01_ = "$2$1";
  
  // снятие курсива со [сноски]
   var re02 = new RegExp("("+sIB+")([\\\.\\\s]{0,2}<A class=note [^<]+?</A>)","gi");
   var re02_ = "$2$1";
  
  // удаление пробела после [сноски] , если за пробелом следует препинание
   var re03 = new RegExp("(\\\s|"+nbspEntity+"){0,1}(<A class=note [^<]+?</A>)(\\\s|"+nbspEntity+"){0,1}(([\\\.,;])|([!\\\?:…])){0,1}(\\\s|"+nbspEntity+"){0,1}("+fIB+"){0,1}","gi");
   var re03_ = "$8$4$2$7";
  
  // удаление пробела перед [сноской]
   var re04 = new RegExp("(\\\s|"+nbspEntity+"){0,1}("+aIB+"){0,1}(\\\s|"+nbspEntity+"){0,1}(<A class=note [^<]+?</A>)","gi");
   var re04_ = "$2$4";
  
  // препинания после [сноски] {в конце строки} везде
   var re05 = new RegExp("(<A class=note [^<]+?</A>)([\\\.!\\\?,:;]+("+fIB+"){0,1})","gi");
   var re05_ = "$2$1";
  
   // препинания после [сноски] в конце строки
   var re06 = new RegExp("(<A class=note href=\\\"[^>]+\\\d{1,4}\\\">[^><]+?</A>)([…]("+fIB+"){0,1})$","gi");
   var re06_ = "$2$1";
  
  // вставка пробела после [сноски] —
   var re07 = new RegExp("(<A class=note [^<]+?</A>)("+sIB+"){0,1}([А-яЁёA-z\\\-–—\\\(\\\[])","gi");
   var re07_ = "$1 $2$3";
  ...
  //~~~~~~~~~~~~~~ Конец шаблонов ~~~~~~~~~~~~~~~~~~
   ...
   function HandleP(ptr) {
    s=ptr.innerHTML;
         ...
         if (s.search(re01)!=-1)         { s=s.replace(re01, re01_); }
         if (s.search(re02)!=-1)         { s=s.replace(re02, re02_); }
         if (s.search(re03)!=-1)         { s=s.replace(re03, re03_); }
         if (s.search(re04)!=-1)         { s=s.replace(re04, re04_); }
         if (s.search(re05)!=-1)         { s=s.replace(re05, re05_); }
         if (s.search(re06)!=-1)         { s=s.replace(re06, re06_); }
  ...
  //    window.external.BeginUndoUnit(document,"«Уборка сносок»");                               // ОТКАТ (UNDO) начало
