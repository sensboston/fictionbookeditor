...
static char isSpace[0xFFFF];
static char isDigit[0xFFFF];
// prepare cache array
static void SetupArrays()
{
	for (int i=0; i<0xFFFF; i++) isSpace[i] = iswspace(i);
}
...
#define IsASpace(ch) isSpace[(unsigned short)ch]

/*inline bool IsASpace(unsigned int ch) {
    return isSpace[ch]==0; // alpha (ch == ' ') || ((ch >= 0x09) && (ch <= 0x0d));
}*/
...
inline bool IsADigit(unsigned int ch) {
	return (iswdigit(ch)==0); // (ch >= '0') && (ch <= '9');
}
