...
const char *RESearch::Compile(const char *pattern, int length, bool caseSensitive, bool posix)
{
	...
	if (re) return 0;
	else return badpat(error);
}

#define OVECCOUNT 300
int RESearch::Execute(const char* subject, int lp, int endp)
{
	...
	if (re)
	{
		rc = pcre_exec(re, NULL, subject+lp, endp-lp, 0, 0, ovector, OVECCOUNT);
		if (rc > 0)
		{
			bopat[0] = ovector[0]+lp;
			eopat[0] = ovector[1]+lp;
            for (int i=1; i<rc; i++)         // added by TaF
					{
						bopat[i] = ovector[i*2]+lp;
						eopat[i] = ovector[i*2+1]+lp;
					}
			return 1;
		}
	}
...
bool RESearch::GrabMatches(const char* subject) {    // TaF
	bool success = true;
		...
		if ((bopat[i] != NOTFOUND) && (eopat[i] != NOTFOUND)) {
			...
			if (pat[i]) {
				for (unsigned int j = 0; j < len; j++)
//					pat[i][j] = ci.CharAt(bopat[i] + j);
					pat[i][j] = subject[bopat[i] + j];   // added by TaF
				pat[i][len] = '\0';
			} else {
				success = false;
			}
