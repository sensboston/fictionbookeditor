...
function Run () {
 ...
 var strConst7="«добавление сноски»";
...
//прочитаем в массив SectID ID-ы секций примечаний
 ...
 if (addSnoska) {
  ...
  newSnoskaNum=j5;                                                         //              MsgBox("newSnoskaNum=j5:  " +newSnoskaNum);
  if (lastSnoskaMode) {
   var it7=newSnoskaNum+1;                                            //               MsgBox("it7:  " +it7+ "\nlinks lenght:  " +document.links.length );
   while (it7<document.links.length)
    if (isItNote(document.links[it7])==false) it7++;
    else break;
   if (it7<document.links.length) {                                   //                MsgBox("it7:  " +it7);     // добавил <=  вместо  <
    document.links[newSnoskaNum].removeNode(true);
    MsgBox("	–= Sclex Script =–\n\n	       Ошибка.\nЭто не последняя сноска в документе!");
    return;
   }
  }
...
var Tsec3 = Math.ceil(1000*((Tf-Ts)/1000-Tmin*60))/1000;
   ...
   if ((EndWindow)&&(addSnoska)) { MsgBox("Добавлена сноска  № " +insertN+ "     \nВремя: "+TimeStr); }
  if ((EndWindow)&&(!addSnoska))
  { MsgBox('      –= Sclex Script =– \n'+
    ' «Унификация сносок» v.'+VersionNumber+'\n\n'+
    'Обработка сносок закончена. \n'+
    'Время: '+TimeStr+ ''); }
