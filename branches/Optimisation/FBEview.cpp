...
bool CFBEView::InsertPoemOrCite(bool fCite, bool fCheck)
{
	try
	{
		...
		if(!ExpandTxtRangeToParagraphs(rng->duplicate(), begin, end))
			return false;
		...
		if(fCite)
		{
			...
			{
				ExpandTxtRangeToParagraphs(rng, begin, end);
				BSTR beginHTML = begin->outerHTML;
				BSTR endHTML = end->outerHTML;
				...
				CString rngHTML = rng->htmlText;

				// Sometimes range returned with DIV even if it's located via markup pointers to Ps
				if(rngHTML.Find(L"<DIV") != -1 && rngHTML.Find(L"</DIV>") != -1)
				{
					int start = rngHTML.Find(L"<DIV");
					int end = rngHTML.Find(L">");
					rngHTML.Delete(start, end - start + 1);
					rngHTML.Delete(rngHTML.Find(L"</DIV>"), 6);
				}

				if(rngHTML.Find(L"<P") != -1)
				{
					CString tempHTML;
					ne->innerHTML = rngHTML.AllocSysString();

					MSHTML::IHTMLElementCollectionPtr els = ne->children;
					for(long i = 0; i < els->length; ++i)
					{
						MSHTML::IHTMLElementPtr cur = els->item(i);
						if(U::scmp(cur->tagName, L"DIV") == 0)
						{
							if(U::scmp(cur->className, L"table") && U::scmp(cur->className, L"poem"))
							{
								if(cur->innerText.GetBSTR() && U::scmp(cur->innerText.GetBSTR(), L" "))
								{
									tempHTML += L"<P>";
									tempHTML += cur->innerText.GetBSTR();
									tempHTML += L"</P>";
								}
							}
							else
								tempHTML += cur->outerHTML.GetBSTR();
						}
						else
						{
							if(!U::scmp(cur->innerText.GetBSTR(), L" "))
								continue;
							tempHTML += cur->outerHTML.GetBSTR();
						}
					}

					ne->innerHTML = tempHTML.AllocSysString();
				}
				else
				{
					rngHTML = CString(L"<P>") + CString(rng->htmlText.GetBSTR()) + CString(L"</P>");
					ne->innerHTML = rngHTML.AllocSysString();
				}
			}
			else
		...
		else
		{
			...
			MSHTML::IHTMLElementPtr sets(Document()->createElement(L"DIV"));
			...
			if (rng->compareEndPoints(L"StartToEnd", rng) != 0)
			{
				...
				{
					CString tempHTML;
					se->innerHTML = rngHTML.AllocSysString();

					MSHTML::IHTMLElementCollectionPtr els = se->children;
					for(long i = 0; i < els->length; ++i)
					{
						MSHTML::IHTMLElementPtr cur = els->item(i);
						if(U::scmp(cur->tagName, L"DIV") == 0)
						...
						else
						{
							if(!U::scmp(cur->innerText.GetBSTR(), L" "))
							{
								if(tempHTML == L"")
									continue;
								MSHTML::IHTMLElementPtr secopy(Document()->createElement(L"DIV"));
								secopy->className = L"stanza";
								secopy->innerHTML = tempHTML.AllocSysString();
								tempHTML = L"";
								MSHTML::IHTMLElement2Ptr(ne)->insertAdjacentElement(L"beforeEnd", secopy);
							}
							else
							{
								tempHTML += cur->outerHTML.GetBSTR();
					...
					if(tempHTML != "")
						se->innerHTML = tempHTML.AllocSysString();
					else
						se->innerHTML = L"";
				}
				else
			...
			ne->className = L"poem";
			if(U::scmp(se->innerHTML.GetBSTR(), L""))
				MSHTML::IHTMLElement2Ptr(ne)->insertAdjacentElement(L"beforeEnd", se);
		}

		MSHTML::IHTMLTxtRangePtr pasted(rng->duplicate());
		pasted->collapse(true);

		RemoveEmptyNodes((MSHTML::IHTMLDOMNodePtr)ne);
		
		BSTR neOHTML = ne->outerHTML;
		if(ne->innerHTML.length() > 0)
		...
		pasted->collapse(false);
		pasted->move(L"character", -1);
		pasted->select();
		m_mk_srv->EndUndoUnit();
	}
