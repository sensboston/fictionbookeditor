// Добавление последнего комментария с вводом
//                                             написал ©Sclex
//                                версия 2.3 от 8 мая 2008 г.
...
function Run () {
 ...
 var strConst7="«добавление с вводом»";
 ...
 if (!body) {MsgBox("–= Sclex Script =–\n\n      Ошибка.\n Body не найдено!"); return;}
 var insertCnt=1;
 ...
 if (addSnoska) {
  ...
  if (!forsazh && j6>=0) {
   ...
   if (abc==-1 || insertN==undefined) {
    document.links[newSnoskaNum].removeNode(true);
    MsgBox("    –= Sclex Script =–\n\n        Ошибка.\n Не удается по ссылке перед вставляемой\n определить номер вставляемого примечания.");
    return;
   }
   insertN++;
  } else insertN=1;
...
//проверим, нет ли в боди нотесов секций второго уровня вложения
     ...
     while (ptr2) {
      if (ptr2.nodeName=="DIV") {
        if (ptr2.className=="section") {
         MsgBox("         –= Sclex Script =–\n\n    В body комментариев есть подсекции.\n Такие файлы не обрабатываются данным скриптом.\n    Работа скрипта завершена.");
         document.links[newSnoskaNum].removeNode(true);
         return;
        }
      }
 ...
 if (InputSnoskaText) {
  var promptText="Ввод текста комментария.\nФорматирование: <b>, <i>, <br>, <strong>, <emphasis>, <em>";
  ...
  while (code!=0) {
   if (code==1) {
     MsgBox("     –= Sclex Script =–\n\n           Ошибка.\n Недопустимый символ в имени тэга.");
   }
   if (code==2) {
     MsgBox("Ошибка. Неразрешенное имя тэга.");
   }
   if (code==3) {
    MsgBox("     –= Sclex Script =–\n\n           Ошибка.\n  Имя закрывающего тэга не соответствует имени открывающего.");
   }
   if (code==4) {
     MsgBox("     –= Sclex Script =–\n\n           Ошибка.\n Не закрыт тэг правой угловой скобкой, как уже начинается новый,\n либо не закрыт последний тэг в строке.");
   }
   if (code==5) {
     MsgBox("     –= Sclex Script =–\n\n           Ошибка.\n Для открытого тэга нет соответствующего закрывающего.");
   }
 ...
 if (Tmin>0) {var TimeStr=Tmin+" мин. "+Tsek+" с"}
  ...
  if ((EndWindow)&&(addSnoska)) { MsgBox("Добавлен комментарий  № " +insertN+ "   \nВремя: "+TimeStr); }
  if ((EndWindow)&&(!addSnoska)) { MsgBox("           –= Sclex Script =–\n Обработка комментариев v." +VersionNumber+ "\n\nОбработка комментариев закончена.\nВремя: "+TimeStr); }
