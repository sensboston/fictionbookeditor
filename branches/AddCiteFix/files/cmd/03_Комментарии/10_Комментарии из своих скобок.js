// извлечение комментариев из разных скобок, определяемых простым текстом
//                                              написал © Sclex, 26 апреля 2008 г.
...
function getVersionStr() {
 return("      –= Sclex Script =–\nКомментарии из скобок v."+versionNumber+"   \n\n");
}
  ...
  if (j5<document.links.length) {
   ...
   while (while_flag) {
    if (j6<document.links.length)
     if (getLocalHref(document.links[j6].getAttribute("href")).search(snoskaHrefRegExp)<0) { j6++; }
...
function finalization() {
 ...
 window.external.BeginUndoUnit(document,"комментарии из скобок");
 if (makePrompt) {
  if (!promptAsText) {
   var s1,s2;
   MasksN=1;
