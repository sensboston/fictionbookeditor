...
public:
  ...
  CCommandBarCtrl		  m_CmdBar1; // menu bar
  ...
  CSimpleMap<int, CString> keycodes;
...
/*    return m_doc->m_desc==m_view.GetActiveWnd() ?
  ...
  BEGIN_MSG_MAP(CMainFrame)
	...
	//COMMAND_CODE_HANDLER(IDN_TREE_MOVE_LEFT_ONE, OnTreeMoveLeftElementOne)
...
//	COMMAND_CODE_HANDLER(IDN_TREE_MOVE_ELEMENT_WITH_CHILDREN, OnTreeMoveElementWithChildren)
	...
	//COMMAND_CODE_HANDLER(IDN_HKMAP_CHANGE, OnHkMapChange)		
  ...
  LRESULT OnFastModeChange(WORD, WORD mode, HWND hWndCtl, BOOL&) 
  {
   ...
   LRESULT OnHkMapChange(WORD, WORD fbw_hk, HWND hWndCtl, BOOL&) 
  {
	  if(fbw_hk)
		  m_hAccel = ::LoadAccelerators(ATL::_AtlBaseModule.GetResourceInstance(), MAKEINTRESOURCE(IDR_MAINFRAME_FBW_SHORTCUTS));
	  else
		  m_hAccel = ::LoadAccelerators(ATL::_AtlBaseModule.GetResourceInstance(), MAKEINTRESOURCE(IDR_MAINFRAME));
	  return 0;
  }
