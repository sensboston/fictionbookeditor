...
public:
	...
	FRBase(FB::Doc* doc): m_view(&doc->m_body), m_doc(doc) { }

	HWND GetDlgItem(int id)
	{
		return X_GetDlgItem(id);
	}
	virtual HWND X_GetDlgItem(int id) = 0;
	BOOL SetDlgItemText(int id, const TCHAR* str)
	{
		return ::SetWindowText(GetDlgItem(id), str);
	}
	...
	void LoadHistoryImp(const TCHAR *path, CRegKey& rk, HWND hCB, CString& first)
	{
		...
		DWORD nfs;
		if(rk.QueryDWORDValue(L"", nfs) != ERROR_SUCCESS)
			return;
	...
	LRESULT OnInitDialog(UINT, WPARAM, LPARAM, BOOL& bHandled)
	{
		...
		LoadHistoryImp(L"SearchHistory", m_fh, GetDlgItem(IDC_TEXT), m_view->m_fo.pattern);
		LoadHistoryImp(L"ReplaceHistory", m_rh, GetDlgItem(IDC_REPLACE), m_view->m_fo.replacement);
...
public:
	...
	CFindDlgBase(FB::Doc *doc) : FRBase(doc) { }
...
class CReplaceDlgBase: public CModelessDialogImpl<CReplaceDlgBase>, public FRBase
{
public:
	enum
	{
	...
	CReplaceDlgBase(FB::Doc *doc) : FRBase(doc), m_selvalid(false) { }
	...
	// change cancel button to "Close"
		::SetWindowText(CModelessDialogImpl<CReplaceDlgBase>::GetDlgItem(IDCANCEL), L"Close");
		SendMessage(DM_SETDEFID, IDC_REPLACE_ONE);
	}
	virtual HWND X_GetDlgItem(int id)
	{
		return CModelessDialogImpl<CReplaceDlgBase>::GetDlgItem(id);
	}
};
...
public:
	HTMLBinder* binder;
	CViewFindDlg(FB::Doc *doc): CFindDlgBase(doc)
	{
		wchar_t* docBody = L"fbw_body";
		MSHTML::IHTMLDOMNodePtr body = MSHTML::IHTMLDocument3Ptr(m_view->Document())->getElementById(docBody);
		binder = new HTMLBinder(L"P", doc, body);
		binder->ParseCurrentNode();
	}
	...
	{
		binder->SetNextNode();
		std::wstring parsed = binder->ParseCurrentNode();
