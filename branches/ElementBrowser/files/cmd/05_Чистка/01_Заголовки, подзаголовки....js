...
function Run() {
 ...
 //  MsgBox("ptr.outerHTML:"+ptr.outerHTML);
  ...
  if (ImageCnt>1 || !section_has_empty_lines_for_validity) {
   ...
   if (first_empty_memorized && (!image_flag || ImageCnt>1))  {
 ...
 window.external.BeginUndoUnit(document,"Заголовки, подзаголовки...");
 if (PromptDestrongTitles) {
  DestrongTitles=DeitalicTitles=AskYesNo("        –= Sclex Script =– \n\n   Нормализовать заголовки?	     ");
 }
...
var Tsec3 = Math.ceil(1000*((Tf-Ts)/1000-Tmin*60))/1000;
  ...
  MsgBox('            –= Sclex Script =– \n'+
        '«Разметка подзаголовков,\n'+
        '             чистка пустых строк» v.'+VersionNumber+'\n  '+

        '\nДвойные пустые строки:		'+EmptyClearedEmpty+
        '\nСоседствующие с цитатами: 	'+EmptyClearedCite+
        '\nСоседствующие с поэмами:	 	'+EmptyClearedPoem+
        '\nВнутри стихотворений:		'+EmptyClearedInPoem+

     '\n\nСмежные с подзаголовками: 	'+EmptyClearedSubtitle+

      '\n\nВ начале секций:			'+EmptyClearedBegin+
        '\nВ конце секций:			'+EmptyClearedEnd+
