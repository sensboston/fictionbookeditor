...
function Run () {
 if (!confirm("Вы запустили скрипт «Перенумеровать комментарии, удаляя неиспользуемые разделы комментариев».\nВы действительно хотите, чтобы неиспользуемые разделы комментариев были удалены?")) return;
...
//создадим заголовок боди нотесов, если нет его
     ...
     while (ptr2) {
      if (ptr2.nodeName=="DIV") {
        if (ptr2.className=="section") {
         MsgBox("В body комментариев есть разделы второго уровня вложенности. Такие документы не обрабатываются данным скриптом. Работа скрипта завершена.");
         if (addSnoska) 
  ...
  if (!forsazh && j6>=0) {
   ...
   if (abc==-1 || insertN==undefined) {
    ...
    MsgBox("Не удалось создать комментарий.\n\n"+
           ...
           "комментариев. Поэтому, чтобы вставить новую сноску, "+
 ...
 if (InputSnoskaText) {
  var promptText="Текст комментария. Используйте <b>...</b> <i>...</i> <br> <strong>...</strong> <emphasis>...</emphasis> <em>...</em>";
